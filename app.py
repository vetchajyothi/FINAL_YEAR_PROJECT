...
def download_file(url, output):
    """Helper to download weights if they don't exist."""
...
def load_models():
    """
    Downloads and initializes models.
    """
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    # Define file paths and URLs
    weights_info = {
        "stroke_classifier_weights.pth": "https://drive.google.com/uc?id=14IgpgBioDyohj8VFbELbTAuRxOsRx0rf",
        "stroke_type_weights.pth": "https://drive.google.com/uc?id=1hGEHj_pcsAzLmUWPUQg0LqpRThnUjB61",
        "unet_weights.pth": "https://drive.google.com/uc?id=1yGySxyxfLMnjWtzO-TwAV9z9gldmZMrc"
    }

    # Execute downloads
    for filename, url in weights_info.items():
    ...
    if os.path.exists("stroke_classifier_weights.pth"):
        model_stroke.load_state_dict(torch.load("stroke_classifier_weights.pth", map_location=device))
         ...
         st.sidebar.error("Stroke Classifier weights missing.")
    model_stroke.eval()
    ...
    if os.path.exists("stroke_type_weights.pth"):
        ...
        st.sidebar.error("Stroke Type weights missing.")
    model_type.eval()
    ...
    if os.path.exists("unet_weights.pth"):
        model_unet.load_state_dict(torch.load("unet_weights.pth", map_location=device))
         ...
         st.sidebar.error("U-Net weights missing.")
    model_unet.eval()
...
def detect_clots_and_lesion(image: Image.Image, conf_threshold: float = 0.5):
    ...
    lesion_area_str = "<br>".join(lesion_area_texts) if lesion_area_texts else "0 px²"
...
def show_data_analysis_report():
    ...
    with col1:
        ...
        data1 = pd.DataFrame({"Class": ["Normal", "Stroke"], "Count": [1240, 1860]}).set_index("Class")
        st.bar_chart(data1)
    with col2:
        st.subheader("Dataset: Stroke Types")
        data2 = pd.DataFrame({"Type": ["Ischemic", "Hemorrhagic"], "Count": [1420, 440]}).set_index("Type")
        st.bar_chart(data2)
    ...
    if uploaded_file is not None:
        ...
        with col2:
            ...
            with m_col1:
                ...
                st.markdown(f'<div class="metric-card"><div class="metric-label">Stroke Prediction</div><div class="{color_class}">{stroke_pred}</div></div>', unsafe_allow_html=True)
            with m_col2:
                ...
                st.markdown(f'<div class="metric-card"><div class="metric-label">Stroke Type</div><div class="{color_class}">{stroke_type}</div></div>', unsafe_allow_html=True)

            st.markdown("<div style='height: 15px'></div>", unsafe_allow_html=True)
            m_col3, m_col4 = st.columns(2)
            with m_col3:
                 color_class = "metric-value-high" if num_clots > 0 else "metric-value-low"
                 st.markdown(f'<div class="metric-card"><div class="metric-label">Blood Clots Detected</div><div class="{color_class}">{num_clots}</div></div>', unsafe_allow_html=True)
            with m_col4:
                color_class = "metric-value-medium" if lesion_area_str != "0 px²" else "metric-value-low"
                st.markdown(f'<div class="metric-card"><div class="metric-label">Lesion Damaged Area</div><div class="{color_class}">{lesion_area_str}</div></div>', unsafe_allow_html=True)

            st.markdown("<div style='height: 15px'></div>", unsafe_allow_html=True)
            r_color = {"High": "metric-value-high", "Medium": "metric-value-medium", "Low": "metric-value-low"}.get(risk_level, "metric-value-neutral")
            st.markdown(f'<div class="metric-card" style="border: 2px solid {"#ff4b4b" if risk_level=="High" else ("#ffa421" if risk_level=="Medium" else "#008f51")};"><div class="metric-label">Risk Prediction Level</div><div class="{r_color}" style="font-size: 32px;">{risk_level} RISK</div></div>', unsafe_allow_html=True)
